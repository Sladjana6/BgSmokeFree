...
function wpse250243_comment_form_default_fields( $fields ) {
    ...
    $fields = [
        'author' => '<p class="comment-form-author">' . '<label for="author">' . __( 'Ime', 'textdomain'  ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
        ...
        'url'    => '<p class="comment-form-url"><label for="url">' . __( 'Vebsajt', 'textdomain'  ) . '</label> ' .
                    '<input id="url" name="url" ' . ( $html5 ? 'type="url"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" maxlength="200" /></p>',
        'comment_field' => '<p class="comment-form-comment"><label for="comment">' . _x( 'Vaš komentar *', 'noun', 'textdomain' ) . '</label> <textarea id="comment" name="comment" cols="45" rows="8" maxlength="65525" aria-required="true" required="required"></textarea></p>',
    ];
...
function my_search_form_text($text) {
     $text = str_replace('value="Search"', 'value="Pretraži"', $text); //set as value the text you want
     return $text;
}
