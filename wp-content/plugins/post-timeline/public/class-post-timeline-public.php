...
class Post_TIMELINE_Public {
	 ...
	 */
  	public function timeline_shortcode( $atts ) {
		...
		//extract( shortcode_atts(array('title_post' => '','category_id' => '','class' => ''), $atts, 'post-timeline' ) );
	...
	public function output_timeline( $ptl_response, $all_configs ) {
		...
		*/
		...
		//	Is Type Post Timeline?
		$is_post_timeline = ($all_configs['ptl-post-type'] == 'post-timeline')? true: false;
		...
		foreach($timeline_posts as $child) {
			...
			if($date) {
				...
				$date_stamp 		= ($is_post_timeline)? strtotime($child->custom['ptl-post-date'][0]): strtotime($child->post_date);
				$child->date_str    = date_i18n($format, $date_stamp);

				$dated = $child->custom['ptl-post-date'][0];
				...
				if(count($dated) == 3) {
					...
					$child->order  	= ($dated[0] * 1000) + ($dated[1] * 100) +  $dated[2];
				}
				else
					$child->order  	= 0;
			}
		...
		$ptl_URL 	  = site_url()."/".POST_TIMELINE_SLUG."/";
		...
		$all_configs['ptl-nav-status']  = (isset($all_configs['ptl-nav-status']) && ($all_configs['ptl-nav-status'] == '0' || $all_configs['ptl-nav-status'] == 'off'))?false:true;
		$all_configs['ptl-anim-status'] = (isset($all_configs['ptl-anim-status']) && ($all_configs['ptl-anim-status'] == '0' || $all_configs['ptl-anim-status'] == 'off'))?false:true;
		$all_configs['ptl-rm-status']   = (isset($all_configs['ptl-rm-status']) && ($all_configs['ptl-rm-status'] == '0' || $all_configs['ptl-rm-status'] == 'off'))?false:true;
		$all_configs['ptl-post-length'] = intval($all_configs['ptl-post-length']);
		
		//dd($all_configs['ptl-rm-status']);

		/*
		if($all_configs['ptl-post-length'] <= 0 || $all_configs['ptl-post-length'] > 1500)
