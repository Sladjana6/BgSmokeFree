...
if(!class_exists('WPMSL_Maps_Frontend_Controller')){
	abstract class WPMSL_Maps_Frontend_Controller{
		public function info_window_content($map_id, $store_id, $content){
	        ...
	        $content = str_replace('{address}','<p><i class="fa fa-external-link-square"></i> '.get_post_meta($store_id,'store_locator_address',true),$content);
			...
			$content = str_replace('{website}','<a href="'.esc_url(get_post_meta($store_id,'store_locator_website',true)).'" class="visit-website">'.__("Visit Website","store_locator").'</a>',$content);
			return $content;
		}
			...
			<div class="col-left leftsidebar slide-left <?php echo isset($grid_options['listing_position']) ? $grid_options['listing_position'].'-skip ' : 'below_map-skip ';  echo isset($grid_options['search_window_position']) ? $grid_options['search_window_position'] : 'wpml_left_map'; ?>">
			    ...
			    <div class="store-search-fields <?php echo $map_window_open?>">
			        ...
			        <div class="store_locator_field">
			        	<div class="map-btns">               
				        <span><input id="store_locatore_search_btn" type="submit" value="<?php  _e("Pronađi", 'store_locator'); ?>" /></span>
					    </div>
			    	</div>
		        </form>
		        </div>
		...
		public function get_direction($meta, $type){
			if($type=='gmap'){
				return "<a href='https://www.google.com/maps?daddr=".$meta['store_locator_lat'].",".$meta['store_locator_lng']."' target='_blank'>".__('Get Direction','store_locator')."</a>";
			}else{
				return "<a class='store-direction' data-direction='".$meta['store_locator_lat'].",".$meta['store_locator_lng']."' style='cursor:pointer;'>".__('Get Direction','store_locator')."</a>";
			}
		}
